...
menu.divider(menu.my_root(), "SnakeScript")
...
menu.action(OnlineRec, "Bounty Loop", {}, "Bigger Lobby, Better Money", function ()
    menu.trigger_commands("bountyall 10000")
    menu.ref_by_path("Players / players.user() / Trolling / Blame For Killing... / All Players")
end)
...
--SnakeScript Options Begin

local SnakeScript = menu.list(menu.my_root(),"SnakeScript", {}, "SnakeScript Options")

menu.action(SnakeScript, "Update SnakeScript", {}, "", function ()
    menu.action(menu.my_root(), "Update SnakeScript", {}, "", function()
        async_http.init('raw.githubusercontent.com','/theracingsnake5/SnakeScript/main/SnakeScript.lua',function(a)
            local err = select(2,load(a))
            if err then
                util.toast("Update failed!")
            return end
            local f = io.open(filesystem.scripts_dir()..SCRIPT_RELPATH, "wb")
            f:write(a)
            f:close()
            util.toast("SnakeScript upadte success!")
            util.toast("Restarting SnakeScript.")
            util.restart_script()
        end)
        async_http.dispatch()
    end)  --credit to jinxscript... i would've never figured this shit out myself
end)

menu.action(SnakeScript, "Restart Script", {}, "Creates a fresh start", function ()
    util.restart_script()
end)